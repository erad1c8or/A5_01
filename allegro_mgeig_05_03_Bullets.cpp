...
//int testing = 0;
int NumBullets = 5;
// !!! Hey ! When I change int NUM_BULLETS to 6 the background changes all green, and when I set it to 10 the loop doesnt even run. Can you tell me why is that ?
...
int main(void)
{
	...
	ALLEGRO_TIMER* gamelooptimer = al_create_timer(1.0 / FPS);		//declare???, create timer and set interval = 1 second / FPS... 1/60 = 60 FPS
...
{
	plyr1.posy += plyr1.speed;
	if (plyr1.posy > DisplayHeight - 17)
		plyr1.posy = DisplayHeight - 17;
}
...
void FireBullet(Bullet bullets[], int size, SpaceShip &ship)
{
	for (int i = 0; i < size; i++)
	{
		if (!bullets[i].live)
		{
			bullets[i].posx = ship.posx + 17;
			bullets[i].posy = ship.posy;
			bullets[i].live = true;
			break;
		}
	}
...
void DrawBullet(Bullet bullets[], int size)	//
{
	...
	{
		if (bullets[i].live)
			al_draw_filled_circle(bullets[i].posx, bullets[i].posy, 2, al_map_rgb(255, 255, 255));
	}
}
...
void UpdateBullet(Bullet bullets[], int size)
{
	for (int i = 0; i < size; i++)
	{
		if (bullets[i].live)
		{
			bullets[i].posx += bullets[i].speed;
			if (bullets[i].posx > DisplayWidth)
				bullets[i].live = false;
		}
	}
}
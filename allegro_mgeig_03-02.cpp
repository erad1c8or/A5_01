...

{
	...
	while (!done)	//while done is not it's default state of false
	{
		...
		else if (event_kbd.type == ALLEGRO_EVENT_DISPLAY_CLOSE)	//detect the display window being closed
		{
			done = true;
		}
	...
	bool doprimitives = true;
	while (!doprimitives)  //while debug is not it's default state of true
	{
	//Text
	al_clear_to_color(al_map_rgb(0, 0, 0));	//Clear the back buffer

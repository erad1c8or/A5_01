...
void CollideBullet(Bullet PlyrBullets[], int NumBullets, Comet comets[], int cometSize);
	...
	*/
	...
	while (!quitapp)	//while variable quitapp is false
	{
		...
		if (event.type == ALLEGRO_EVENT_TIMER)	//update position of player every frame
		{
			...
			CollideBullet(PlyrBullets, NumPlyr1Bullets, comets, NumComets);
...
void CollideBullet(Bullet someBullets[], int NumBullets, Comet comets[], int cometSize)
{
	for (int i = 0; i < NumBullets; i++)
	{
		if (someBullets[i].live)
